<template>
	<div class="bg-neutral-800 border border-neutral-700 rounded-lg p-6">
		<div class="flex items-center justify-between mb-4">
			...
			<button @click="clearHistory" class="text-gray-400 cursor-pointer hover:text-gray-300 text-sm" title="Limpar histórico">Limpar</button>
		</div>
		...
		<div class="mb-6">
			...
			<div class="flex gap-2">
				...
				/>
				<button @click="rollCustom" class="bg-blue-600 hover:bg-blue-700 cursor-pointer text-white px-4 py-2 rounded-md text-sm transition-all duration-300">Roll</button>
			</div>
		</div>
		...
		<div v-if="rollHistory.length > 0">
			...
			<div class="max-h-48 overflow-y-auto space-y-2">
				<div v-for="(roll, index) in rollHistory.slice().reverse()" :key="index" class="bg-neutral-900 border border-neutral-700 rounded-md p-3">
